...
describe("Restaurants class", function () {
  ...
  it("reports that Kayasa Restaurant is not open just beefore 8:30 am", () => {
    expect(getRestaurantsOpenAt({weekday: 0, hour: 7, minute: 29}))
      .toEqual([]);
  });

  it("reports that Kayasa Restaurant and WBS closes on Sunday 9pm", () => {
    expect(getRestaurantsOpenAt({ weekday: 6, hour: 20, minute: 59})
      .filter((item) => item === "World's Best Steakhouse"))
      .toEqual(["World's Best Steakhouse"]);

    expect(getRestaurantsOpenAt({ weekday: 6, hour: 21, minute: 0})
      .filter((item) => item === "World's Best Steakhouse"))
      .toEqual([]);
  })
  ...
  it("reports that WBS is not open at or after Sunday 1am", () => {
    ...
    expect(getRestaurantsOpenAt({ weekday: 6, hour: 1, minute: 1 }))
      .toEqual([]);
  });

  it("reports that WBS is open on Sunday 12am, +/- 1 minute", () => {
    expect(getRestaurantsOpenAt({ weekday: 6, hour: 0 }))
      .toEqual(["World's Best Steakhouse"]);
    expect(getRestaurantsOpenAt({ weekday: 5, hour: 23, minute: 59 }))
      .toEqual(["World's Best Steakhouse"]);
    expect(getRestaurantsOpenAt({ weekday: 6, hour: 0, minute: 1 }))
      .toEqual(["World's Best Steakhouse"]);
  });
});
